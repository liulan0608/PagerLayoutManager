...
public class PageIndicatorView extends LinearLayout {
    ...
    public void initIndicator(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            view.setImageResource(R.drawable.shape_circle_light_blue);
            addView(view, params);
            indicatorViews.add(view);
        }
        if (indicatorViews.size() > 0) {
            indicatorViews.get(0).setImageResource(R.drawable.shape_circle_blue);
        }
    }
    ...
    public void setSelectedPage(int selected) {
        for (int i = 0; i < indicatorViews.size(); i++) {
            if (i == selected) {
                indicatorViews.get(i).setImageResource(R.drawable.shape_circle_blue);
            } else {
                indicatorViews.get(i).setImageResource(R.drawable.shape_circle_light_blue);
            }
        }
    }
